...
class GeminiProvider extends BaseModelProvider<GeminiConfig> {
  ...
  async getDefaultModels(): Promise<ModelList> {
    ...
    data.models.forEach((m: any) => {
      // Only include models that support the NEW embedContent method
      // (LangChain uses embedContent, not the deprecated embedText)
    ...
    // Sort embedding models to prioritize recommended models
    // text-embedding-004 is the recommended model for most use cases
    defaultEmbeddingModels.sort((a, b) => {
      if (a.key.includes('text-embedding-004')) return -1;
      if (b.key.includes('text-embedding-004')) return 1;
      if (a.key.includes('gemini-embedding-001')) return -1;
      if (b.key.includes('gemini-embedding-001')) return 1;
      return 0;
    });
